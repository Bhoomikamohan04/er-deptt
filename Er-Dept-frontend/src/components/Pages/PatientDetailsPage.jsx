...
const ROLE_CONFIG = {
  admin: {
    ...
    canViewReports: true,
    options: ["medicalforms", "billing", "labresults", "radiologyresults", "appointments"],
  },
  doctor: {
    canViewAll: false,
    canEditPatientDetails: true,
...
const PatientDetailsPage = () => {
  ...
  const generateDummyPatient = () => {
    const firstNames = ['John', 'Jane', 'Robert', 'Emily', 'Michael', 'Sarah', 'David', 'Lisa', 'James', 'Emma'];
    const lastNames = ['Smith', 'Johnson', 'Williams', 'Brown', 'Jones', 'Miller', 'Davis', 'Garcia', 'Rodriguez', 'Wilson'];
    const genders = ['Male', 'Female', 'Other'];
    const bloodGroups = ['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'];
    const conditions = ['Hypertension', 'Diabetes', 'Asthma', 'Arthritis', 'Migraine', 'None'];
    const doctors = ['Dr. Sharma', 'Dr. Patel', 'Dr. Kumar', 'Dr. Gupta', 'Dr. Singh'];
    ...
    const address = `${Math.floor(Math.random() * 1000) + 1} ${['Main St', 'Oak Ave', 'Pine Rd', 'Maple Dr', 'Cedar Ln'][Math.floor(Math.random() * 5)]}, ${['Mumbai', 'Delhi', 'Bangalore', 'Hyderabad', 'Chennai'][Math.floor(Math.random() * 5)]}`;
    ...
    return {
      ...
      age: age,
      gender: gender,
      dob: new Date(new Date().setFullYear(new Date().getFullYear() - age)).toISOString().split('T')[0],
      phone: phone.replace(/(\d{3})(\d{3})(\d{4})/, '$1-$2-$3'),
      address: address,
      aadhaar: aadhaar,
      condition: conditions[Math.floor(Math.random() * conditions.length)],
      assignedTo: doctors[Math.floor(Math.random() * doctors.length)],
      admission_date: new Date(Date.now() - Math.floor(Math.random() * 30) * 24 * 60 * 60 * 1000).toISOString(),
      blood_group: bloodGroups[Math.floor(Math.random() * bloodGroups.length)],
      vitals: {
        blood_pressure: `${Math.floor(90 + Math.random() * 40)}/${Math.floor(60 + Math.random() * 30)}`,
  ...
  useEffect(() => {
    const fetchPatient = async () => {
      ...
      const dummyPatient = generateDummyPatient();
      setPatient(dummyPatient);
      
      // Uncomment the following code to fetch from the database instead
      /*
      ...
      if (!error) {
        ...
        const { data: userData, error: userError } = await supabaseclient
          ...
          .eq("mrno", mrno)
          .single();
        if (!userError) setPatient(userData);
      }
      */
    };
    fetchPatient();
  ...
  const handleSavePatient = async () => {
    ...
    try {
      ...
      showError("Failed to update patient details");
      console.error(err);
    }
  };
      ...
      <div className="max-w-8xl mx-auto flex gap-6">
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4 mb-6">
            ...
            <button className="bg-white p-4 rounded-xl shadow-md hover:shadow-lg transition-all border border-slate-100 hover:scale-105">
              <Scan className="w-8 h-8 text-red-600 mb-2 mx-auto" />
              <p className="font-semibold text-slate-900">Radiology Results</p>
            </button>
            <button className="bg-white p-4 rounded-xl shadow-md hover:shadow-lg transition-all border border-slate-100 hover:scale-105">
            ...
            <button 
              ...
              className="bg-white p-4 rounded-xl shadow-md hover:shadow-lg transition-all border border-slate-100 hover:scale-105"
            >
              <FileText className="w-8 h-8 text-indigo-600 mb-2 mx-auto" />
                ...
                <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-100">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-slate-900 font-medium">{patient.condition || 'N/A'}</p>
                    </div>
                    <div>
                      <label className="text-xs text-slate-500 font-medium">Assigned Doctor</label>
                      <p className="text-slate-900 font-medium">{patient.assignedTo || 'N/A'}</p>
                    </div>
                    <div>
                      <label className="text-xs text-slate-500 font-medium">Admission Date</label>
                      <p className="text-slate-900 font-medium">
                        {patient.admission_date ? 
                          format(new Date(patient.admission_date), 'MMM d, yyyy - hh:mm a') : 
                          'N/A'}
                      </p>
                    </div>
                    <div>
                      <label className="text-xs text-slate-500 font-medium">Blood Group</label>
                      <p className="text-slate-900 font-medium">{patient.blood_group || 'N/A'}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-100">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="bg-slate-50 p-4 rounded-lg border border-slate-100">
                      ...
                      <p className="text-xl font-bold text-slate-900">
                        {patient.vitals?.spo2 ? `${patient.vitals.spo2}%` : 'N/A'}
                      </p>
                    </div>
                  </div>
                </div>
